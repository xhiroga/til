...
<!-- 交差エントロピーを図形で書くとどうなるのか調べる。予想としては、2つの確率分布が同じ場合は調和の取れた形になるはずだ、という予想から始めると理解しやすそうだ -->
...
状態の変化が直前の状態のみに依存し、かつ状態が離散的なものをマルコフ連鎖 (Markov chain)といい、状態が連続的なものをマルコフ過程 (Markov process)ということが多い。[^yasuda_kakukatei]
[^yasuda_kakukatei]: [確率過程](https://www.math.s.chiba-u.ac.jp/~yasuda/sysKOU/kakuKatei.pdf)

観測可能な状態が隠れた状態に依存しているモデルを隠れマルコフモデルという。例えば、遠距離に住んでいる友達と毎日通話して、その日に起きた出来事を聞く。聞いた内容（例えば、散歩や読書）から、友達の住んでいる場所の天気を推測する。これは隠れマルコフモデル的と言える。[^satomacoto_2008]
[^satomacoto_2008]: [隠れマルコフモデルの例](https://satomacoto.blogspot.com/2008/07/blog-post.html)
