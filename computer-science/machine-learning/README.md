...
パラメトリックモデルは確率分布を指すが、ここでは広くパラメータに則って動く数式を指す。また、数式を定め、評価手法を定めてからパラメータを予測する手法そのものも指す。

ここでは多項式を最小二乗法・最尤推定法によって近似する方法を述べる。

||多項式（誤差が正規分布）|多項式（誤差がなんか別のやつ）
|-|-|-|
|最小二乗法による誤差E_Dの式||
|最小二乗法による、最小化するための誤差E_D|平方根平均二乗誤差|
|最小二乗法による誤差E_Dを最小化するために解析する式|辺微分係数をゼロにするやつ|
|最尤推定法の誤差の式|正規分布の分散＝平方根二乗誤差|なんか違う分散
|最尤推定法で最小化する関数|(ラージパイのやつ)|（ラージパイのやつ）
|最尤推定法で解析する式|（偏微分係数をゼロにするやつ）|
...
回帰ではなく分類を解けるモデル。最小化するための式は解析的には解けないため、何度も繰り返しで最小化する。

- モデル: （2値分類の場合）バイアス項、X,Yを足すやつ、0以上か以下かで分類する
- 対象の式: （誤って分類した値に対して）その値の合計を対象とする
- ステップ内の操作: W_new = W_old - 重みを各Wについて微分した値
...
確率関数は、モデルの数式をロジスティックの数式に入れたもの
目的関数は、ラージパイで確立を掛け合わすやつ
最大化の方法は、IRISといって、勾配降下法を更に高度にしたものらしい

パーセプトロン・ロジスティック回帰の良さは、PR曲線とROC曲線で測る

TODO: 適合率、そうじゃない方の覚え方
...
- [[改訂新版]ITエンジニアのための機械学習理論入門](https://amzn.to/3yPDrrU)
